...
The library window (`ManagementView.swift`) is split by wallpaper target. **Home Screen** is the only interactive one — its grid sets `activeWallpaperID` and drives `WallpaperWindow`. **Screen Saver** is read-only: `WallpaperStoreRegistrar.currentScreenSaverSelection(library:)` decodes `~/Library/Application Support/com.apple.wallpaper/Store/Index.plist` (`AllSpacesAndDisplays → Idle → Content → Choices[0]`, whose `Configuration` is an embedded binary plist holding `{"assetID": …}`) to report what macOS currently has selected; selection itself happens in System Settings. **That file is only ever read — never write it.** **Lock Screen** is an informational row: macOS has no separate lock screen wallpaper, so there is nothing to control. The menu action "Remove Hazel Wallpapers from System Settings…" is the only caller of `removeAll()`, and its effect is undone by the next launch-time sync — it exists for use before uninstalling.
